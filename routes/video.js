...
router.post("/", upload.fields([
    ...
    try {
        ...
        const insertResult = await insertVideo(videoUploadResult, req.body.title, req.body.description, req.body.tags ? req.body.tags.split(",") : [], req.body.category, thumbnailUploadResult, parseFloat(req.body.duration));
        if (!insertResult) {
            return res.status(500).send("Something went wrong!");
        }
...
router.get('/stats', async (req, res) => {
    try {
        const page = parseInt(req.query.page) || 1;
        const pageSize = 5;
        const timestamp = req.query.timestamp || 'all';
...
router.get('/most-viewed', async (_req, res) => {
    try {
...
router.get('/least-viewed', async (_req, res) => {
    try {
